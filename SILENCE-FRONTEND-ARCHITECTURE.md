...
```
silence-core/
├── 01_governance/           # Canonical policies, ADRs, brand book
│   ├── SILENCE-BRAND-BOOK-LITE.md
│   ├── SILENCE-FRONTEND-ARCHITECTURE.md
│   ├── HARD_SEVEN_v2026.md
│   ├── RULE-DOM-001_P0_FULL_FILE.md
│   └── decisions/
│       └── ADR-001-frontend-tech-stack.md
├── 02_infrastructure/       # Terraform, CI/CD base configs
├── 03_ee/                   # Enterprise (off-limits for frontend)
├── 04_packages/             # Shared, open-core packages
├── 05_apps/                 # Applications (web, mobile, admin)
├── 06_infrastructure/       # Deploy configs, Docker, etc.
├── 07_research/             # Research artifacts (reference-only)
├── .github/
│   ├── workflows/
│   │   ├── ci.yml
│   │   ├── deploy-web.yml
│   │   ├── deploy-mobile-eas.yml
│   │   └── release.yml
│   └── dependabot.yml
├── pnpm-workspace.yaml
├── turbo.json
...
│   │   │   └── index.ts
│   │   ├── css/
│   │   │   └── variables.css (generated from src/)
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── types/
│   │   ├── src/
│   │   │   ├── user.ts (UserProfile interface)
│   │   │   ├── events.ts (SilenceEventV1, EventBus)
...
│   ├── contracts/
│   │   ├── src/
│   │   │   ├── sdk.ts (SDK public API contracts)
│   │   │   ├── telemetry.ts (trackSilenceEvent signature)
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── ui/
│   │   ├── src/
│   │   │   ├── components/
│   │   │   │   ├── Button.tsx (web + RN variants)
│   │   │   │   ├── Input.tsx
│   │   │   │   ├── Modal.tsx
│   │   │   │   ├── Card.tsx
│   │   │   │   └── index.ts
│   │   │   ├── web/ (web-specific overrides)
│   │   │   ├── native/ (React Native overrides)
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── guards/
│   │   ├── src/
│   │   │   ├── s11-validator.ts (enforce S11 vocabulary)
│   │   │   ├── boundary-check.ts (verify no EE imports)
│   │   │   ├── math-core-validator.ts (verify φ timings)
│   │   │   ├── effectlog-append.ts (append-only log helper)
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   └── sdk/
...
│       │   │   ├── useUpdateProfile.ts
│       │   │   └── index.ts
│       │   ├── telemetry.ts (trackSilenceEvent implementation)
│       │   └── index.ts
│       ├── package.json
│       └── tsconfig.json
```

### 3.3 05_apps/web Structure
...
│   ├── intent-selector/
│   │   ├── page.tsx (step 1: pick intention)
│   │   ├── layout.tsx
│   │   └── components/ (local to this route)
│   ├── observation-baseline/
│   │   ├── page.tsx (step 2: calibration)
│   │   └── components/
│   ├── dashboard/
│   │   ├── page.tsx (main UI: TodayHeroInsight, MoodCard, etc.)
│   │   ├── layout.tsx (sidebar nav)
│   │   └── components/
│   ├── garden/ (φ-Garden sandbox)
│   │   ├── page.tsx
│   │   ├── layout.tsx
│   │   └── components/
│   ├── api/
│   │   ├── health/route.ts (ping endpoint)
│   │   └── webhook/effectlog/route.ts (optional: receive EffectLog from backend)
│   └── (auth)/
│       ├── login/page.tsx
│       ├── signup/page.tsx
│       └── callback/page.tsx (OAuth redirect)
├── lib/
│   ├── config.ts (environment, API URLs)
│   ├── hooks/ (custom React hooks shared across routes)
│   │   ├── useProfile.ts (wraps SDK)
│   │   ├── useSessions.ts
│   │   ├── useTheme.ts (Soft Noir variant switcher)
│   │   └── index.ts
│   ├── stores/ (Jotai atoms for global state)
│   │   ├── auth.ts (current user, token)
│   │   ├── profile.ts (user preferences, intent)
│   │   ├── sessions.ts (cached list of sessions)
│   │   └── index.ts
│   ├── utils/
│   │   ├── formatters.ts (date, time, numbers)
│   │   ├── validators.ts (form validation, S11 checks)
│   │   └── math-helpers.ts (φ, Fibonacci calculations)
│   ├── telemetry.ts (calls @silence/sdk trackSilenceEvent)
│   └── constants.ts (S11 vocabulary map, error messages)
├── components/
│   ├── common/ (reusable across entire app)
│   │   ├── Header.tsx
│   │   ├── Sidebar.tsx
│   │   ├── SafetyBanner.tsx (crisis alert, sticky)
│   │   └── ThemeSwitcher.tsx
│   ├── layout/ (page structure components)
│   │   ├── TwoColumnLayout.tsx (61.8/38.2 split)
│   │   └── CardGrid.tsx (Bento layout)
│   └── ui/ (re-exports from @silence/ui)
├── styles/
│   ├── globals.css (CSS resets, φ-token injection)
│   ├── variables.css (dark mode variants: graphite, ember, midnight, ion)
│   └── animations.css (φ-derived durations, easing)
├── public/
│   ├── fonts/ (JetBrains Mono woff2)
│   └── icons/ (Feather Icons as SVG)
├── package.json
├── tsconfig.json
├── next.config.js (Vercel, image optimization)
├── tailwind.config.js (optional: if using Tailwind, but prefer CSS vars)
└── jest.config.js
```

### 3.4 05_apps/mobile Structure
...
│   │   ├── signup.tsx
│   │   └── callback.tsx (OAuth)
│   └── _sitemap.ts (deep linking)
├── src/
│   ├── components/ (reusable components, web + RN compatible)
│   │   ├── common/
│   │   │   ├── SafetyBanner.tsx
│   │   │   ├── Header.tsx
│   │   │   └── ThemeSwitcher.tsx
│   │   └── ui/ (re-exports from @silence/ui)
│   ├── hooks/ (custom hooks, same as web where possible)
│   │   ├── useProfile.ts
│   │   ├── useSessions.ts
│   │   └── index.ts
│   ├── stores/ (Jotai atoms, same as web)
│   │   ├── auth.ts
│   │   ├── profile.ts
│   │   └── index.ts
│   ├── lib/
...
│   │   └── constants.ts
│   ├── styles/
│   │   └── index.ts (React Native StyleSheet + φ-tokens)
│   └── database/ (React Native SQLite for offline storage)
│       ├── schema.ts
│       ├── migrations.ts
│       └── db.ts
